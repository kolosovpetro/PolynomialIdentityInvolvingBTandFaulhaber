...
\end{equation*}
it is very important to note that the summation bound is $p$ while binomial coefficient upper index is $p+1$.
It means that we cannot skip summation bounds unless we use some trick as
\begin{equation*}
    \begin{split}
        \sum_{k=1}^{n} k^{p}
...
\end{equation}
An elegant proof of the above binomial identity is provided in~\cite{scheuer2023mathstackexchange}.
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Now let's simplify the double summation by applying the identity~\eqref{eq:combinatorial-identity}
\begin{equation*}
    \begin{split}
        \sum_{k=1}^{n} k^{r} (n-k)^{r}
...
The coefficients $\coeffA{m}{r}$ are also registered in the OEIS~\cite{oeis_numerators_of_the_coefficient_a_m_r,oeis_denominators_of_the_coefficient_a_m_r}.

...
Such a system of linear equations is constructed via expanding the definition of the coefficients $\coeffA{m}{r}$
(reference) applying Binomial theorem and Faulhaber's formula.
...
\end{equation}
Applying the Faulhaber's formula to $\sum_{k=1}^{n} k^{t+r}$ we get
\begin{equation}
    \begin{split}
        n^{2m+1}
        &= \sum_{r=0}^{m} \coeffA{m}{r} \left[ \sum_{t=0}^{r} (-1)^t \binom{r}{t} n^{r-t} \sum_{k=1}^{n} k^{t+r} \right] \\

We begin our mathematical journey from investigation of the pattern in terms of finite differences
$\Delta$ of cubes $n^3$.
Consider the table of finite differences $\Delta$ of the polynomial $n^3$
\begin{table}[H]
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \caption{Table of finite differences $\Delta$ of $n^3$} \label{tab:table}
\end{table}
It is easy to observe that finite differences $\Delta$ of polynomial $n^3$ may be expressed according
to the pattern
\begin{align*}
...
\end{align*}
Furthermore, the polynomial $n^3$ turns into
\begin{align*}
...
\end{equation}
Assume that equation~\eqref{eq:cube_identity} has an implicit form such as
\begin{equation}
    \label{eq:pattern}
    n^3 = \sum_{k=1}^{n} \coeffA{1}{1} k^1(n-k)^1 + \coeffA{1}{0} k^0(n-k)^0,
\end{equation}
where $\coeffA{1}{1} = 6$ and $\coeffA{1}{0} = 1$, respectively.
The main problem we meet is to answer to the question:
\textit{Could the pattern~\eqref{eq:pattern} be generalised for all positive odd powers?}
Let be a conjecture
\begin{conj}
